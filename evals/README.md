...
repetitions, budget, latency, provider attempts and retries, failed edit
attempts, stop reasons, permission and rejection counts, failure classification,
and ACP usage or cost when supplied. Missing usage remains `null`. Provider
...
prompts. The overlay remains outside the agent workspace until the phase runs.
