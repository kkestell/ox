...
// The class Ur.AgentLoop.AgentLoop lives inside the Ur.AgentLoop namespace,
// causing a name clash when `using Ur.AgentLoop;` is in scope. Alias it so
// the test body can say `Loop` without ambiguity.
using Loop = Ur.AgentLoop.AgentLoop;

// Suppress the "no parameters" warning for EnumeratorCancellation on zero-arg async iterators
// used in SpyChatClient.
#pragma warning disable CS8424

namespace Ur.Tests.AgentLoop;
