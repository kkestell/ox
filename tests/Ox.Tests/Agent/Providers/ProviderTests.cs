...
using Ox.Agent.Providers.Google;
using Ox.Agent.Providers.Ollama;
using Ox.Agent.Providers.OpenAI;
using Ox.Agent.Providers.OpenAiCompatible;
using Ox.Agent.Providers.OpenRouter;
using Ox.Agent.Providers.ZaiCoding;
...
public sealed class ProviderTests
{
    ...
    [Fact]
    public void OpenAi_ConfigureChatOptions_IsNoOp()
    {
        // OpenAI's Chat Completions endpoint rejects reasoning_effort + tools on
        // GPT-5.x with HTTP 400, and the live agent loop always attaches tools.
        // Until the provider moves to the Responses API we keep ConfigureChatOptions
        // a no-op — this test locks that contract in so a regression can't silently
        // re-enable reasoning here and break normal turns.
        var provider = new OpenAiProvider(new TestKeyring());
