...
namespace Ox.Tests.Agent.Settings;

/// <summary>
/// Tests for the configuration pipeline that replaced <c>SettingsLoader</c>:
/// <see cref="OxSettingsConfigurationProvider"/> (load + migration) and
...
public sealed class SettingsConfigurationTests : IDisposable
{
    ...
    {
        var userPath = WriteTempJson("""{"ox": {"color": "blue"}}""");
        var workspacePath = WriteTempJson("""{"ox": {"color": "red"}}""");

        var config = BuildConfig(userPath, workspacePath);

        Assert.Equal("red", config["ox:color"]);
    }
    ...
    {
        var userPath = WriteTempJson("""{"ox": {"model": "openai/gpt-4o"}}""");
        var config = BuildConfig(userPath, null);
        var options = config.GetSection("ox").Get<OxOptions>();
    ...
    {
        // Verifies that binding OxOptions from the "ox" section sees the new
        ...
        var before = config.GetSection("ox").Get<OxOptions>();
        ...
        writer.Set("ox.model", modelValue, ConfigurationScope.User);

        var after = config.GetSection("ox").Get<OxOptions>();
    ...
    private static IConfigurationRoot BuildConfig(string? userPath, string? workspacePath)
    {
        ...
        builder.Add(new OxSettingsConfigurationSource(userPath));
        if (workspacePath is not null)
            builder.Add(new OxSettingsConfigurationSource(workspacePath));
