...
namespace Ox.Tests.Agent.Logging;

public sealed class OxFileLoggerTests : IDisposable
{
    ...
    private string TodayLogPath =>
        Path.Combine(_logDir, $"ox-{DateTime.Now:yyyy-MM-dd}.log");
    ...
    {
        var logger = new OxFileLogger("Test", _logDir);
    ...
    {
        var logger = new OxFileLogger("MyApp.Service", _logDir);
    ...
    {
        using var provider = new OxFileLoggerProvider();
