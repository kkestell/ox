...
public sealed class ScreenDumpWriterTests : IDisposable
{
    ...
    [Theory]
    [InlineData(Ox.App.ScreenDumpWriter.DumpKeyCode, true)]
    [InlineData(Ox.App.ScreenDumpWriter.DumpFallbackKeyCode, true)]
    [InlineData(71, false)]
    public void IsDumpShortcut_RecognizesSupportedKeys(int keyCode, bool expected)
    {
        var isShortcut = Ox.App.ScreenDumpWriter.IsDumpShortcut(keyCode);
    ...
    public void Write_CreatesArchiveAndLatestFiles()
    {
        ...
        var archivePath = Ox.App.ScreenDumpWriter.Write(_workspacePath, screenText, now);
