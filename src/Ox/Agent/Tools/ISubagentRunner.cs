...
///
/// Lives in the Agent layer.Tools alongside its sole consumer (SubagentTool) so the Tools
/// namespace has no dependency on Ox.Agent.AgentLoop. The concrete SubagentRunner
/// lives in the Agent layer.AgentLoop and implements this interface — it already depends on
/// Ox.Agent.Tools for ToolRegistry, so the dependency is one-directional.
///
