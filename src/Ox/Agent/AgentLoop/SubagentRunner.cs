...
/// <see cref="ISubagentRunner"/> — the interface lives in the Agent layer.Tools alongside
/// its consumer (SubagentTool). SubagentRunner already depends on Ox.Agent.Tools
/// for ToolRegistry, so implementing the interface adds no new dependency.
///
/// Architecture: SubagentRunner mirrors what <see cref="Ox.Agent.Sessions.OxSession"/>
    ...
    Action<ChatOptions>? configureChatOptions = null) : ISubagentRunner
{
    ...
    public async Task<string> RunAsync(string task, CancellationToken ct)
    {
        ...
        // copy) keeps the two sides of this contract in sync — Ox.Agent.Tools is already
