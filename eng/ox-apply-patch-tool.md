...
exactly. An update that produces the same contents is a successful no-op.
...
Use `N path` for a no-op update. A `Failed` outcome includes the
phase—arguments, parse, preflight, or apply—and the relevant path or chunk when
...
- Cover empty patches and files, blank added lines, multiple chunks, append,
  deletion of all content, move-only, edited move, and no-op update.
