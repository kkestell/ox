...
- Compatibility handling for the existing text-based transcript format.
...
described in section 16 before those tools become available.
...
These properties apply to newly written data. Legacy records must be validated
and normalized before being treated as accepted history.
...
cannot be opened is a startup error. Missing credentials are not a startup
error: the process must remain available for terminal authentication.
...
are reported; they are not treated as a missing key.
...
After a successful terminal login saves a key, the next authenticated request
can populate an empty slot without a process restart. Explicit login validates
the key using the provider's non-generation authentication operation before
saving it. A successful key check is not a guarantee that every model request
will succeed.
...
authenticated operation may load it again; report this behavior clearly.
...
struct AssistantMessage {
    ...
    continuation: Option<OpenRouterContinuation>,
}
...
`parts` preserves the supported content structure and order exposed by the
adapter's assembled result. It is not a record of network arrival order, which
can interleave fragments without defining semantic message order.
...
The provider-qualified continuation field is deliberately concrete. Preserve
its complete JSON values, nested fields, association, and array order rather
than interpreting opaque elements as display text. JSON formatting and object
key order are not a byte-preservation contract. Unknown fields inside opaque
metadata are retained; unknown transcript formats remain errors.

Every newly accepted assistant message has a producing model. The optional
field permits legacy text-only messages whose producing model was never
recorded; it is not an excuse to omit the model from new writes. Continuation
metadata requires a known producing model. Retaining that identity prevents a
later configuration change from silently reinterpreting incompatible data.
Unsupported continuation is an actionable error, not a reason to drop metadata
...
Legacy repair may use a narrowly scoped private transaction helper. Do not
expose unrestricted event append to the prompt loop merely to support repair.

`read` distinguishes an absent session from an existing untitled session.
`append_user` and `append_batch` fail for an absent session; they never create
one implicitly. Empty batch writes are not used to change activity.
...
### Codec and transactions

Private database DTOs carry an explicit format version for new structured
assistant messages. They are distinct from both domain structs and HTTP DTOs.
A provider adapter change must not silently change the meaning of old records.
...
Read validation rejects malformed payloads, unknown record versions, orphan
results, duplicate results, and unresolved calls outside the explicitly
supported legacy repair case. Do not skip unreadable rows or substitute empty
history.
...
Load holds its claim through read, validation or repair, replay, and response
enqueue. Delete holds its claim through the database operation and response
enqueue. Neither holds the registry mutex while doing that work.
...
appearing in the UI after model acceptance, when Ox has a complete call to
represent. This avoids provisional tool objects that need repair if model
assembly fails.
...
1. Reads the session and committed transcript.
2. Validates and, if permitted, repairs legacy history under the session claim.
3. Reconstructs the accepted model conversation.
4. Appends the user message and title/activity update in one transaction.
5. Adds that same user message to its owned accepted history.
6. Enqueues the session metadata update.
7. Enters the model/tool loop.

No model request occurs before step 4 succeeds. Preparation failures before
that point require an error response and guard release but no batch settlement.
After step 4, every ordinary exit goes through settlement, even when there is
...
Each call also tracks whether its initial and terminal client updates were
successfully enqueued. These small delivery flags are transient and do not
determine the durable result. They let settlement finish presentation without
duplicating terminal updates or assuming the client has seen an unannounced
call.
...
while another model call is allowed:
    ...
    if tool calls are allowed:
        ...
        execute the call or record its validation failure
...
Before starting a call, check cancellation and enqueue its started update.
If delivery fails, do not start that call; settle it as not started. While the
call runs, observe cancellation according to that tool's resource lifecycle.
As soon as a result is obtained, record it in the pending batch before sending
its terminal update or checking whether later work should begin.
...
7. Attempt any remaining terminal tool updates when the connection is usable.
8. Enqueue the final response or request error.
9. Release admission by dropping the guard.

Repair notifications follow repair persistence so failure of the connection
cannot prevent the repair from being saved. Normal live tool updates may have
been emitted earlier; settlement does not need to re-emit terminal updates that
were already enqueued successfully.

For a call that was never announced, such as one rejected by the model-call
budget, emit a complete terminal tool-call object containing its identity,
arguments, and outcome. Do not send an update that assumes the client already
knows that call, and do not suggest that unstarted work was running. For a call
whose initial update was enqueued, emit its terminal update normally.
...
| Model-call budget reached | Close accepted unexecuted calls with budget failures and commit. | Model-request-limit stop. |
...
requested workspace matches the stored workspace, performs any permitted
legacy repair, and replays the resulting transcript. Workspace validation
precedes any repair write or replay. The claim remains held until the load
...
Use shared constructors for visible text, reasoning, tool starts, and terminal
tool states. Live text uses deltas; replay uses stored complete segments.
Structured messages may project to several updates, so the conversion API
must not assume one transcript record always equals one ACP update.

Replay emits only displayable content and tool states. Opaque continuation
data is intentionally omitted. Completed tools replay as completed; failed
and cancelled tools map to the protocol's available terminal failure state
...
## 15. Existing transcript compatibility

Existing databases may contain a flat text-based format with these kinds:
user messages, assistant thought text, assistant answer text, individual tool
calls, and individual tool results. Tool results can be completed, failed, or
cancelled; an omitted status in the supported old format means completed.

Retain decoding for those established shapes and add a distinct versioned
kind for new structured assistant messages. Do not serialize the new domain
types over the old kind names or delete existing databases to simplify startup.
All new assistant writes use the structured format.

The compatibility reader groups an unambiguous contiguous assistant run and
its associated calls before the corresponding result boundary. It preserves
thought and answer text as parts and associates the calls with that same
message. Paired results remain in call order. It does not attach earlier
thought text to an unrelated later answer.

Some original message boundaries and provider metadata were not stored. The
reader cannot recover them exactly. Where grouping is ambiguous or required
continuation metadata is absent, report the limitation rather than guess a
message structure or fabricate provider data. The supported legacy path is
text-only continuation where the stored content is sufficient.

An unambiguous trailing legacy batch may contain calls with missing results.
Under the prompt or load claim, append failed results explaining that the
previous execution ended before its outcome was recorded and any effects are
unknown. Commit the repair before replay or inference. A second read must not
add duplicate repairs.

Orphan results, conflicting call identifiers within a batch, and unresolved
calls buried before subsequent exchanges are actionable transcript errors.
They are not candidates for an automatic guessed rewrite. New structured
batches with missing results are corruption, not the legacy repair case.

The old format does not identify its producing model. Its normalization is
limited to the known text-only compatibility scope; it must not pretend to
have modern model-qualified continuation metadata.

Compatibility code is isolated at the codec and normalization boundary. The
new execution loop consumes validated structured history, not two independent
conversation models.

## 16. Durability boundary and future mutating tools
...
| Wrong workspace on load | Explicit mismatch error before replay or repair. |
...
Tests establish behavior at ownership and persistence boundaries. They do not
need to freeze private helper organization or reproduce every enum constructor.

### Adapter fixtures

Use a local HTTP server and deterministic response fixtures to verify request
encoding, stream assembly, response acceptance, failures, and cancellation.
Include text, multiple calls, fragmented call arguments, structured continuation
metadata, malformed responses, incomplete termination, and a failure after
visible output. Assert that partial calls never execute and completion is
accepted at most once.

Test credential validation separately from inference. Fixtures should prove
protocol handling without making paid model requests. A manual live smoke test
can validate the supported model integration, but it is not a substitute for
deterministic boundary tests.

### Store and codec

Use in-memory stores for normal operations and actual temporary database files
for close/reopen tests. Verify:

- Metadata and history are read from one snapshot.
- User append adopts a title only once and updates activity atomically.
- Batch append preserves one assistant message and all paired results.
- Failure during a transaction leaves no partial batch.
- Structured continuation data survives encode, save, reopen, and decode.
- Unknown kinds, malformed versions, and broken call/result relationships fail
  explicitly.
- Session deletion removes its records and repeated deletion succeeds.
- Legacy grouping preserves supported associations and trailing repair is
  idempotent.

An in-memory readback is not a restart test. At least one test must close the
file-backed store, construct a new store, and reconstruct history from it.

### Admission

Use controlled futures or barriers rather than timing sleeps. Verify both
orders of prompt/load, prompt/delete, and load/delete conflicts; duplicate
prompt and duplicate load rejection; guard release on normal completion,
early error, and dropped future; and cancellation isolation across sessions.

Creating or loading session B must not replace session A's history, redirect
its notifications, or cancel its prompt. Cancellation before the first poll
must remain observable by the admitted prompt.
...
or fail a chosen enqueue. No sink trait is needed.

Use the concrete model client with local fixtures to drive the real loop.
Test pure pending-batch settlement directly where that gives a smaller test.
Do not create a provider trait solely to supply mocks.

Required cases include:

- Normal text answer and accepted history advancement after commit.
- Several tool calls in one assistant message with ordered terminal results.
- Unknown tool and invalid arguments followed by a model correction.
- Cancellation before model acceptance, during a tool, and between tools.
- Preservation of a result already observed when cancellation arrives.
- Provider failure after provisional text, leaving no accepted partial answer.
- Delivery failure after an observed result, preserving that result in a
  successfully settled batch.
- Delivery failure before tool start, preventing that tool from running.
- Failed batch append, leaving accepted in-memory history unchanged.
- Final allowed model call requesting tools, with no tool execution and
  explicit budget results.
- Request error handling followed by a successful prompt for another session.

### End-to-end acceptance

Create a session and retain the returned ID. Submit a tool-using prompt,
cancel after assistant acceptance but before all results are obtained, load
the session, close the process/store, reopen the same database, and reconstruct
the next model request. Assert both ACP replay and encoded model history.
There must be no unresolved call, duplicate result, misplaced reasoning, or
interleaved replay.

Run a second sequence carrying actual-shaped structured continuation metadata
through save, reopen, and tool follow-up. Visible reasoning alone is not proof
that continuation metadata survived.

Verify terminal login followed by a successful retry in the same running
process. Verify logout behavior with and without an environment key and with
an already running prompt.

## 19. Implementation sequence

1. Define the structured conversation records, private versioned DTOs, and
   closed-batch validation. Implement the persistent store and compatibility
   reader with focused storage tests.
...
3. Implement atomic session admission and the unique operation guard. Test its
   conflict matrix and cancellation ownership.
...
7. Run focused boundary tests and the end-to-end restart and authentication
   sequences, then perform the appropriate Rust build and test checks.
...
## 20. Tradeoffs and extension triggers
