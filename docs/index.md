...
- **Linear startup via `UrHost.Start`.** Workspace → load model cache (or fetch) → extension manifest discovery/schema registration → settings load/validate → initialize effective-enabled extensions → return `UrHost`. Missing API key or model selection does not fail startup; those surface as chat-readiness blockers on the configuration/runtime API. Disabled extensions remain manifest-only until the user enables them.
- **Dedicated extension management state.** Extension enablement is not stored in repo-scoped settings. Global overrides live under `~/.ur/`; workspace extension opt-ins live in per-user workspace state keyed by workspace path hash. This preserves the trust model behind workspace-default-disabled extensions. See [ADR-0012](decisions/adr-0012-extension-enablement-state-and-lazy-activation.md).
...
| Extension Management | Resolves effective extension state, persists enable/disable overrides, and exposes a UI-safe management surface. | [extension-management.md](extension-management.md) |
