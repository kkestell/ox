...
- A new test is justified only by a distinct, durable, observable guarantee or
  a reproduced regression that existing coverage cannot express clearly. New
  code, another branch, another input row, or a plan item is not by itself a
...
- Give each guarantee one owner. Do not repeat the same assertions in unit,
  ...
  failure modes. Table-driven cases exercising one behavior belong in one
  test.
- Do not game the count by combining functions while retaining duplicated
  setup and assertions. Optimize the maintenance surface, not the reported
  number of tests.
- Before finishing any change that touches tests, inspect the complete test
  diff and report the test-function and test-code delta. Any net growth needs a
...
- Every fixed bug should be covered when practical, preferably by strengthening
  an existing test. Add a separate regression test only when no existing test
  can express the failure clearly.
